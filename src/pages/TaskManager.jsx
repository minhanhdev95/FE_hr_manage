...
import { Table, Button, Tag, Space, Modal, Form, Input, Select, DatePicker, message, Card, Row, Col, Tooltip, Alert, Descriptions } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined, SearchOutlined, ReloadOutlined, EyeOutlined, SyncOutlined } from '@ant-design/icons';
...
const TaskManager = () => {
  ...
  const refresh = () => fetchData(pagination.current, pagination.pageSize, currentFilters);
  ...
  const getRecordClass = (record) => {
    const name = record.trangThaiTen
  ...
  const handleSaveForm = async () => {
    try {
      ...
      const choPheDuyetId = resolveTrangThaiId(statuses, TRANG_THAI.CHO_PHE_DUYET);
      const payload = {
        ...values,
        uuid: editingId,
  ...
  const openUpdateStatus = (record, cls) => {
    ...
    const firstNextId = nextClasses.length ? resolveTrangThaiId(statuses, nextClasses[0]) : null;
  ...
  const buildPayload = (record, overrides = {}) => ({
    ...
    loaiCongViecId: record.loaiCongViecId != null ? Number(record.loaiCongViecId) : null,
    noLucThucHien: record.noLucThucHien,
    ngayBatDau: record.ngayBatDauString
      || (record.ngayBatDau ? dayjs(record.ngayBatDau).format('DD-MM-YYYY') : null),
    ngayKetThuc: record.ngayKetThucString
      || (record.ngayKetThuc ? dayjs(record.ngayKetThuc).format('DD-MM-YYYY') : null),
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      await taskService.update(buildPayload(statusRecord, { trangThaiId: nextStatusId }));
  ...
  const confirmDelete = (record) => {
    Modal.confirm({
      ...
      cancelText: 'Hủy',
      onOk: () => taskService.delete(record.uuid).then(() => {
  ...
  const productLabel = (record) => record.sanPhamTen
    || products.find((p) => p.value === Number(record.sanPhamId))?.label
    || record.sanPhamId || '-';
  const typeLabel = (record) => record.loaiCongViecTen
    || types.find((t) => t.value === Number(record.loaiCongViecId))?.label
    || record.loaiCongViecId || '-';
  const statusLabel = (record) => record.trangThaiTen
    || statuses.find((s) => s.value === Number(record.trangThaiId))?.label
    || '';
  ...
  const columns = [
    ...
    { title: 'Sản phẩm', render: (_, record) => productLabel(record), width: 150 },
    { title: 'Loại CV', render: (_, record) => typeLabel(record), width: 150 },
    {
      title: 'Trạng thái',
      width: 140,
      render: (_, record) => {
        const cls = getRecordClass(record);
        return <Tag color={TRANG_THAI_TAG_COLOR[cls]}>{statusLabel(record) || 'N/A'}</Tag>;
      },
    },
    {
      title: 'Thao tác',
      ...
      render: (_, record) => {
        const cls = getRecordClass(record);
        const canEdit = cls === TRANG_THAI.CHO_PHE_DUYET || cls === TRANG_THAI.TU_CHOI;
        const canUpdateStatus = !!(NEXT_STATUS_MAP[cls] && NEXT_STATUS_MAP[cls].length);
        const canDelete = cls === TRANG_THAI.CHO_PHE_DUYET || cls === TRANG_THAI.TU_CHOI;
        return (
          <Space size={0}>
            <Tooltip title="Xem chi tiết">
              <Button icon={<EyeOutlined />} type="text" onClick={() => openDetail(record)} />
            </Tooltip>
            {canEdit && (
              <Tooltip title={cls === TRANG_THAI.TU_CHOI ? 'Sửa & xin duyệt lại' : 'Chỉnh sửa'}>
                <Button icon={<EditOutlined />} type="text" onClick={() => openEdit(record, cls)} />
              </Tooltip>
            )}
            {canUpdateStatus && (
              <Tooltip title="Cập nhật trạng thái">
                <Button icon={<SyncOutlined />} type="text" onClick={() => openUpdateStatus(record, cls)} />
              </Tooltip>
            )}
            {canDelete && (
              <Tooltip title="Xóa">
                <Button icon={<DeleteOutlined />} danger type="text" onClick={() => confirmDelete(record)} />
              </Tooltip>
            )}
          </Space>
        );
  ...
  const nextStatusOptions = (NEXT_STATUS_MAP[statusRecordClass] || []).map((k) => ({
  ...
  return (
    <div style={{ padding: '24px', background: '#f5f7f9', minHeight: '100vh' }}>
      <Card style={{ marginBottom: '16px' }} styles={{ body: { padding: '20px' } }}>
        <Form form={filterForm} layout="vertical">
          <Row gutter={[16, 0]}>
            ...
            <Col span={16} style={{ display: 'flex', alignItems: 'flex-end', paddingBottom: '24px' }}>
              <Space>
      ...
      <Card
        title={<b>CÔNG VIỆC CỦA TÔI</b>}
        styles={{ body: { padding: '0px' } }}
        extra={<Button type="primary" icon={<PlusOutlined />} onClick={openAdd}>Thêm mới</Button>}
      >
        <Table
          columns={columns}
          dataSource={data}
      ...
      >
        ...
        <Form form={form} layout="vertical" style={{ marginTop: 8 }}>
          <Form.Item name="noiDungCongViec" label="Nội dung" rules={[{ required: true, message: 'Vui lòng nhập nội dung' }]}>
          ...
          <Row gutter={16}>
            ...
            <Col span={12}><Form.Item name="sanPhamId" label="Sản phẩm"><Select options={products} placeholder="Chọn sản phẩm" allowClear /></Form.Item></Col>
            <Col span={12}><Form.Item name="loaiCongViecId" label="Loại công việc"><Select options={types} placeholder="Chọn loại" allowClear /></Form.Item></Col>
            <Col span={12}><Form.Item name="noLucThucHien" label="Nỗ lực"><Input placeholder="Nhập nỗ lực..." /></Form.Item></Col>
      ...
      >
        {detailRecord && (
          <Descriptions bordered column={2} size="small" styles={{ label: { width: 140 } }}>
            <Descriptions.Item label="Mã CV">{detailRecord.maCongViec || '-'}</Descriptions.Item>
            <Descriptions.Item label="Trạng thái">
              <Tag color={TRANG_THAI_TAG_COLOR[getRecordClass(detailRecord)]}>{statusLabel(detailRecord) || 'N/A'}</Tag>
            </Descriptions.Item>
            <Descriptions.Item label="Nội dung" span={2}>{detailRecord.noiDungCongViec || '-'}</Descriptions.Item>
            <Descriptions.Item label="Sản phẩm">{productLabel(detailRecord)}</Descriptions.Item>
            <Descriptions.Item label="Loại công việc">{typeLabel(detailRecord)}</Descriptions.Item>
            <Descriptions.Item label="Nỗ lực">{detailRecord.noLucThucHien || '-'}</Descriptions.Item>
            <Descriptions.Item label="Ngày bắt đầu">{detailRecord.ngayBatDauString || (detailRecord.ngayBatDau ? dayjs(detailRecord.ngayBatDau).format('DD-MM-YYYY') : '-')}</Descriptions.Item>
            <Descriptions.Item label="Ngày kết thúc" span={2}>{detailRecord.ngayKetThucString || (detailRecord.ngayKetThuc ? dayjs(detailRecord.ngayKetThuc).format('DD-MM-YYYY') : '-')}</Descriptions.Item>
          </Descriptions>
        )}
      </Modal>
