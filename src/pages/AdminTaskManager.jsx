...
const AdminTaskManager = () => {
  ...
  const handleSubmitApprove = async () => {
    ...
    try {
      await adminService.update({
        uuid: editTask.uuid,
        trangThaiId: selectedTrangThaiId,
      });
      ...
      const currentFilters = {
        ...currentValues,
        ngayBatDau: currentValues.rangeDate
          ? currentValues.rangeDate[0].format("DD-MM-YYYY")
          : null,
        ngayKetThuc: currentValues.rangeDate
          ? currentValues.rangeDate[1].format("DD-MM-YYYY")
          : null,
      };
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      setUserLoading(true);
      try {
        const [resUsers, resProducts, resTypes, resStatuses] =
          await Promise.all([
  ...
  const columns = [
    ...
    {
      title: "Trạng thái",
      dataIndex: "trangThaiTen",
      render: (t, record) => (
        <Tag {...getStatusTagProps(record.trangThaiId)}>{t}</Tag>
      ),
      width: 140,
    },
    ...
    { title: "Ngày kết thúc", dataIndex: "ngayKetThucString", width: 180 },
    {
      ...
      render: (_, record) => {
        const isPending =
          record.trangThaiTen === "Chờ phê duyệt" || record.trangThaiId === 1;
        return (
          <Space>
            {isPending && (
              <Button
    ...
    <div style={{ padding: "24px" }}>
          ...
          >
            <div style={{ padding: "12px", borderBottom: "1px solid #f0f0f0" }}>
              <Checkbox
                onChange={(e) =>
                  setSelectedUserIds(
                    e.target.checked ? users.map((u) => u.uuid) : [],
                  )
                }
                checked={
          ...
          <Card style={{ marginBottom: 16 }}>
            <Form form={filterForm} layout="vertical">
              <Row gutter={[16, 0]}>
                ...
                <Col span={6}>
                  <Form.Item name="loaiCongViecId" label="Loại CV">
                    <Select
          ...
          >
            <Table
              ...
              pagination={{
                ...
                onChange: (p) =>
                  fetchTasks(p, pagination.pageSize, selectedUserIds),
              }}
        ...
        <Form form={giaoViecForm} layout="vertical">
          <Form.Item
            ...
            label={
              <span>
                {" "}
                Nội dung công việc <span style={{ color: "red" }}>*</span>
              </span>
            }
          >
            ...
            <Col span={12}>
              <Form.Item
                ...
                label={
                  <span>
                    {" "}
                    Nhân sự <span style={{ color: "red" }}>*</span>
                  </span>
                }
              >
                <Select placeholder="Chọn nhân sự">
                  {users.map((u) => (
                    <Select.Option key={u.uuid} value={u.uuid}>
                      {u.hoTen}
                    </Select.Option>
                  ))}
                </Select>
            ...
            <Col span={12}>
              <Form.Item
                ...
                label={
                  <span>
                    {" "}
                    Nỗ lực <span style={{ color: "red" }}>*</span>
                  </span>
                }
              >
      ...
      >
        <Form form={editForm} layout="vertical">
          ...
          <Form.Item name="trangThaiId" label="Trạng thái">
            <Select
              value={selectedTrangThaiId}
              onChange={(v) => setSelectedTrangThaiId(v)}
            >
              {(statuses || [])
                .filter(
                  (s) =>
              ...
              {!(statuses || []).some((s) => s.id === 2) && (
                <Select.Option value={2}>Đã tiếp nhận</Select.Option>
              )}
              {!(statuses || []).some((s) => s.id === 3) && (
          ...
          <Form.Item label="Nội dung công việc">
            <TextArea
              rows={3}
              value={editTask?.noiDungCongViec || ""}
              disabled
            />
          </Form.Item>
        </Form>
      ...
      <Modal
        ...
        footer={
          <Button onClick={handleCloseDetailModal} icon={<CloseOutlined />}>
            Đóng
          </Button>
        }
        width={700}
        ...
        {selectedTask && (
          <Form layout="vertical">
            ...
            <Form.Item label="Nội dung công việc">
              <TextArea
                rows={4}
                value={selectedTask.noiDungCongViec || ""}
                disabled
              />
            </Form.Item>
            <Row gutter={16}>
            ...
            <Row gutter={16}>
              <Col span={12}>
                <Form.Item label="Ngày bắt đầu">
                  <DatePicker
                    value={
                      selectedTask.ngayBatDauString
                        ? dayjs(selectedTask.ngayBatDauString, "DD-MM-YYYY")
                        : null
                    }
                    disabled
                    style={{ width: "100%" }}
              ...
              <Col span={12}>
                <Form.Item label="Ngày kết thúc">
                  <DatePicker
                    value={
                      selectedTask.ngayKetThucString
                        ? dayjs(selectedTask.ngayKetThucString, "DD-MM-YYYY")
                        : null
                    }
                    disabled
                    style={{ width: "100%" }}
